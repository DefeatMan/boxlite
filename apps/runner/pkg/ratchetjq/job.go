...
// StatusFailed reports that the run raised instead of producing an outcome,
// and is paired with the error text on the way out (spec §2.4).
//
// No job type sets it on a Result: a job that failed returns an error and a nil
// Result, which is the only shape ReportFunc allows for a failure. It is
// Report that turns that error into this status, because that is the layer
// which knows how a failure is spelled on the wire
// (transfer/control_plane.go:Report).

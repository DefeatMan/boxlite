...
// A run that raised is an outcome: the control plane records why, its accept
// segment gets to compensate, and the reason stops living only in this
// process's logs.
func TestReportPostsAFailedRunWithItsErrorText(t *testing.T) {
	transfer, requests := newControlPlane(t, http.StatusCreated, `{"accepted":true}`)
	job := ratchetjq.Job{ID: "job-1", Type: "echo"}

	err := transfer.Report(context.Background(), job, nil, errors.New("the box was gone"))
	if err != nil {
		t.Fatalf("Report returned %v, want no error", err)
	}
	...
	if report.body["status"] != string(ratchetjq.StatusFailed) {
		t.Fatalf("report sent status %v, want %s", report.body["status"], ratchetjq.StatusFailed)
	}
	if report.body["errMsg"] != "the box was gone" {
		t.Fatalf("report sent errMsg %v, want the run error", report.body["errMsg"])
	}
	if _, sent := report.body["outParams"]; sent {
		t.Fatalf("report sent outParams %v for a run that produced none", report.body["outParams"])
	}
}

// A run cut short because its own round ended has not failed — the lease lapsed
// and the row may already have been handed to another round, so an outcome
// posted for it could overwrite work this runner no longer owns.
func TestReportSaysNothingWhenTheRoundWasLost(t *testing.T) {
	for name, runErr := range map[string]error{
		"lease lapsed":    context.DeadlineExceeded,
		"runner stopping": context.Canceled,
		"wrapped":         fmt.Errorf("calling the guest: %w", context.DeadlineExceeded),
	} {
		t.Run(name, func(t *testing.T) {
			transfer, requests := newControlPlane(t, http.StatusCreated, `{"accepted":true}`)

			if err := transfer.Report(context.Background(), ratchetjq.Job{ID: "job-1"}, nil, runErr); err != nil {
				t.Fatalf("Report returned %v, want no error", err)
			}
			if len(*requests) != 0 {
				t.Fatalf("the control plane saw %d requests, want none", len(*requests))

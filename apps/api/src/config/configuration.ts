...
export function ratchetjqConfig(env: NodeJS.ProcessEnv = process.env) {
  const settings = {
    ...
    maxUnfinishedPerExecutor: requiredCount(
      env.RATCHETJQ_MAX_UNFINISHED_PER_EXECUTOR,
      1_000,
      'RATCHETJQ_MAX_UNFINISHED_PER_EXECUTOR',
    ),
  }
  ...
  // A queue shallower than one claim can drain is a limit that refuses work the
  // executor was about to take anyway: the first claim after a submission may pull
  // a whole batch, so anything at or below that is a ceiling on throughput rather
  // than on backlog.
  if (settings.maxUnfinishedPerExecutor <= settings.claimBatchSize) {
    throw new Error(
      `RATCHETJQ_MAX_UNFINISHED_PER_EXECUTOR must exceed RATCHETJQ_CLAIM_BATCH_SIZE (${settings.claimBatchSize}), ` +
        `got "${settings.maxUnfinishedPerExecutor}"`,
    )
  }

...
export class RatchetJQScannerPool {
  ...
  /**
   * Whether the pool is short of its floor, so a process starting up should offer
   * a Scanner.
   *
   * Not the negation of `isAboveFloor`: that one is `> floor` and this one is
   * `< floor`, so a pool sitting exactly at the floor is neither — which is right,
   * since that is the state both are there to preserve. Nobody may retire from it
   * and nobody needs to be added to it.
   */
  async isBelowFloor(): Promise<boolean> {
    return (await this.liveCount()) < this.configService.get('ratchetjq.scannerMinN')
  }

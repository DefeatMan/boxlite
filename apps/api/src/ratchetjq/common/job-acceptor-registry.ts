...
import { IJobAcceptor, isAsyncJobAcceptor, isSyncJobAcceptor } from './job-acceptor'
 ...
 * (`apps/runner/pkg/ratchetjq/jobs/registry.go:22`), reached through Nest
 ...
 * (`apps/runner/pkg/ratchetjq/factory.go:65`): that one calls `Attach` because a
...
export class RatchetJQJobAcceptorRegistry {
   ...
   * the first job that names the type: an empty name, an acceptor implementing
   * neither form, and a second claim on a name already taken. Private because
   * the injected list is the only way in — an acceptor registered later would be
   * one the Scanner had already turned away.
   */
  private register(acceptor: IJobAcceptor): void {
    if (!acceptor.type) {
      throw new Error('A RatchetJQ job acceptor reported an empty type name')
    }
    if (!isSyncJobAcceptor(acceptor) && !isAsyncJobAcceptor(acceptor)) {

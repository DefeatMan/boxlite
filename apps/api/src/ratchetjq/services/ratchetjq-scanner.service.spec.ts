...
import { IJobAcceptor, ISyncJobAcceptor, RatchetJQAcceptVerdict } from '../common/job-acceptor'
...
/** The three statements one round runs, in the order `runRound` calls them. */
const STATEMENTS_PER_ROUND = 3

/**
 * Builds the Scanner over stubs.
 *
 * `rows` is what each successive selecting statement returns, as row arrays; the
 * loop runs three per round — force-advance `pending_run`, force-advance
 * `running`, retry `accepting` — so entries are consumed in threes.
 *
...
function makeScanner(options: {
  ...
  return { service, query, pool, registry, finalizer, configService, retired, sleepReads, selects, roundCount }
}
...
class StubAcceptor implements ISyncJobAcceptor {
  seen: string[] = []
  signals: AbortSignal[] = []
  ...
  async syncAccept(job: RatchetJQJob, signal: AbortSignal): Promise<RatchetJQAcceptVerdict> {
 ...
 */
class SilentAcceptor implements ISyncJobAcceptor {
  readonly type = 'silent'
  signals: AbortSignal[] = []

  syncAccept(_job: RatchetJQJob, signal: AbortSignal): Promise<RatchetJQAcceptVerdict> {
    this.signals.push(signal)
 ...
 */
class CancellableAcceptor implements ISyncJobAcceptor {

...
function makeScanner(options: {
  ...
  retireFails?: Error
  // What the startup check reads, and whether it can read it at all.
  belowFloor?: boolean
  floorCheckFails?: Error
}) {
  ...
  const pool = {
    ...
    isBelowFloor: jest.fn(async () => {
      if (options.floorCheckFails) {
        throw options.floorCheckFails
      }
      return options.belowFloor ?? false
    }),
...
describe('RatchetJQScannerService startup', () => {
  // Without this the pool is only ever grown by a report, so a service that has
  // just deployed and has no traffic yet runs no Scanner — which is exactly when a
  // backlog left by the previous processes has nobody looking at it.
  it('offers a Scanner when the pool is short of its floor', async () => {
    const { service, pool, retired } = makeScanner({ belowFloor: true, aboveFloor: [true] })

    await service.onApplicationBootstrap()
    await retired

    expect(pool.isBelowFloor).toHaveBeenCalled()
    expect(pool.acquire).toHaveBeenCalledWith(expect.any(String))
  })

  // At the floor is not below it. A rolling deploy where every replica offered one
  // would fill the pool to its ceiling and leave the idle-retire path to unwind it.
  it('offers nothing when the pool already meets its floor', async () => {
    const { service, pool, query } = makeScanner({ belowFloor: false })
  ...
  // The pool says who is scanning; it is not something this process needs in order
  // to serve traffic, so an unreachable Redis must not stop the API from starting.
  it('starts the process anyway when the pool cannot be read', async () => {
    const { service, pool } = makeScanner({ floorCheckFails: new Error('redis unreachable') })

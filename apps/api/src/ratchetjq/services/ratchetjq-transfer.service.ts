...
import { RATCHETJQ_ERR_MSG_MAX_CHARS, RatchetJQJob } from '../entities/ratchetjq-job.entity'
 ...
 *
 * A status, and then one of two things depending on which it is: the output for
 * a run that produced one, or the error text for a run that raised. They are
 * separate fields rather than one payload because a round only ever has one of
 * them — a run that raised produced no output — and collapsing them would leave
 * every reader inspecting the value to find out which it was holding.
 *
 * A failure is an outcome here, which it was not before: an executor whose run
 * raised reports `status = failed` and says why, instead of staying silent and
 * leaving the lease to redeliver a job that will raise again. What that costs is
 * the redelivery — a reported failure ends the run segment and hands the job to
 * the accept segment, so a round that failed transiently no longer gets the rest
 * of its `ATTEMPT_MAXN` rounds. The judgement is moved rather than removed: it
 * is the acceptor that now decides whether a failure stands or is compensated,
 * which is the side that knows what the job was for.
 *
 * Only the run that raised is reported. A run cut short because its own lease
 * lapsed is not a failure of the job but the scheduler taking it back, so it
 * stays silent and is redelivered exactly as before
 * (`apps/runner/pkg/ratchetjq/transfer/control_plane.go:Report`).
 */
...
export class RatchetJQTransferService {
   ...
   * completes without one (`accept-finalizer.ts:complete`).
   *
   ...
   *
   * `errMsg` is written on the same statement as the status it belongs to, so a
   * row never holds `failed` without the reason or a reason without the failure.
   * It is written unconditionally rather than only for a failure: a report that
   * carries no error clears the column, which matters because this row may
   * already hold one — a job pushed inline that raised, was left claimable, and
   * then succeeded on the pulled path would otherwise complete as `ok` still
   * displaying the error from the round it recovered from.
   */
  async report(
    executor: string,
    executorId: string,
    ...
    const [job] = await this.jobRepository.query(
      ...
      [
        ...
        this.boundedErrMsg(outcome.errMsg),
      ],
    )
    if (!job) {
  ...
  /**
   * Cancel a job by spending the rounds it has left (spec §8.5, the timeout
   * trigger, reached early).
   *
   * A cancellation is not a terminal state of its own. Writing `attempt` past
   * `ATTEMPT_MAXN` puts the row in exactly the state a job that has exhausted its
   * retries is in, and every statement then does what it already does: no retry
   * touches it — the claim retry, the force-advance and the accept retry all test
   * `attempt <= ATTEMPT_MAXN` — and the Scanner's sweep retires it as `timeout`
   * behind a rollback job once its lease lapses. Nothing here has to know how a job
   * ends, which is what keeps this on TRANSFER: creating a compensation would mean
   * depending on the writer, and the writer already depends on this service for the
   * wake-up hint, so that edge would close a cycle Nest refuses to build.
   *
   * The price is that it is not immediate. A cancelled `running` row is retired on
   * the sweep's cadence rather than at once, and the round already dispatched runs
   * to its end — there is no channel from here to a job in flight on a runner, so
   * that is true of any cancellation this side can express.
   *
   * **A round that reports in time undoes it**, which is the one gap worth stating:
   * `report` moves the row to `accepting` and writes `attempt = 2`, giving the
   * accept segment its own budget (§2.2), so an executor that answers before the
   * lease lapses resets the counter and the job carries on to be accepted. Cancel
   * is therefore best-effort against the round already running, and reliable only
   * for the rounds that have not started.
   *
   * `pending_run` needs one thing more, because for that stage the counter alone
   * does nothing at all: the advance statements write `attempt = 2` unconditionally
   * and the sweep never looks at `pending_run` (§3), so a bumped row would be
   * dispatched with a fresh counter and the cancellation would vanish. It is moved
   * to `running` in the same statement — which is not a special case but the route
   * §8.3 already established, and for the same reason: `pending_run` is never
   * abandoned directly, so force-advance exists precisely to put a job where
   * rollback can reach it. A row parked there with its rounds spent is inert, since
   * every claim tests `attempt <= ATTEMPT_MAXN`, and its lease is already `now()`
   * from insert, so the next sweep takes it.
   *
   * It goes to the sweep rather than being completed here on the grounds that it
   * never ran, because that reasoning is wrong: `rollbackType` compensates the
   * operation, not the executor's part of it, and a submitter generally has state
   * of its own that it wrote before submitting. A cancelled job gets its
   * compensation like any other job that ends as `timeout`.
   *
   * The deadlines are deliberately left alone. Forcing `leaseExpiresAt` to `now()`
   * would have the sweep retire a `running` job while its executor is still inside
   * the deadline it was granted — queueing the compensation while the side effect
   * it undoes is still being produced. Letting the lease lapse on its own means the
   * rollback is created after the executor's window has closed.
   *
   * One statement, because the branch has to be decided against the row as it is at
   * that instant. Reading first and writing second would let a claim or a report
   * move the row in between and apply the wrong arm.
   */
  async cancel(jobId: string): Promise<RatchetJQJob | null> {
    const attemptMaxN = this.configService.get('ratchetjq.attemptMaxN')

    const [job] = await this.jobRepository.query(
      `WITH cancelled AS (
         UPDATE "ratchetjq_job"
         SET "attempt" = $2,
             -- Reads the row's period from before this statement, so the test and
             -- the assignment cannot disagree.
             "period" = CASE WHEN "period" = $3 THEN $4 ELSE "period" END
    ...
    if (!job) {
      // Either there is no such job, or it has already completed. Both mean there
      // is nothing to cancel, and the caller is better placed than this statement
      // to know which it was expecting.
      this.logger.warn(`No unfinished RatchetJQ job ${jobId} to cancel`)
      return null
    }

    this.logger.log(
      `Cancelled RatchetJQ job ${jobId}; it is ${job.period} with its rounds spent, ` +
        `for the Scanner to retire and roll back`,
    )
  ...
  /**
   * Fits an executor's error text to the column that stores it.
   *
   * This is the boundary the text crosses: it was composed by another service,
   * about a failure whose shape nobody here controls, and `errMsg` is declared
   * `varchar(RATCHETJQ_ERR_MSG_MAX_CHARS)` — so an over-long message reaching the
   * statement would fail the report and lose an outcome that was correctly
   * delivered. Cutting it is the lesser loss.
   *
   * The head is what survives, and the ellipsis is there so a reader can tell a
   * cut message from an executor that phrased it that way. Keeping the head does
   * cost the root cause of a wrapped Go chain, which reads outermost-first — but
   * a chain that long is a stack or an echoed payload rather than a sentence, and
   * the wrapper still names the operation that failed.
   *
   * Empty is null, not an empty string: an executor that reported no text has
   * told us nothing, and the column already has a way to say that.
   */
  private boundedErrMsg(errMsg: string | null | undefined): string | null {
    if (!errMsg) {
      return null
    }
    if (errMsg.length <= RATCHETJQ_ERR_MSG_MAX_CHARS) {
      return errMsg
    }

    return `${errMsg.slice(0, RATCHETJQ_ERR_MSG_MAX_CHARS - 1)}…`
  }

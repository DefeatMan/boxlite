 ...
 *
 * Both are orchestration and nothing else. Rows are written by
 * RatchetJQJobWriter, the push is whichever ISyncJobExecutor serves the kind the
 * submission names, and the accept segment belongs to RatchetJQReportService.
 * That split is what lets this service reach the accept segment at all: the
 * segment has to create rollback jobs, so it depends on the writer — if it
 * depended on PROPOSER instead, the graph would close on itself and Nest would
 * refuse to build it.
 */
@Injectable()
export class RatchetJQProposerService {
   ...
   * same reading `timeout: 0` has in axios (`ratchetjq-runner.executor.ts:63`) and

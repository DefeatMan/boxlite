 ...
 */
function makeService(options: { claimed?: unknown[]; durSeconds?: number | null; reported?: unknown[] } = {}) {
  ...
  const durCall = (): QueryCall =>
    query.mock.calls.find(([sql]) => !sql.includes('WITH claimed AS') && !sql.includes('WITH reported AS')) as QueryCall
  const reportCall = (): QueryCall => query.mock.calls.find(([sql]) => sql.includes('WITH reported AS')) as QueryCall

  return { service, query, redis, brpop, disconnect, claimCalls, durCall, reportCall }
}

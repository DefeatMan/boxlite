 ...
 */
@Injectable()
export class RatchetJQScannerService implements OnApplicationBootstrap, OnApplicationShutdown {
   ...
   * refills a slot freed by a crash — nothing tops it up on a timer. Two callers
   * reach it: a report, which is the path that creates the work a Scanner is for,
   * and this process starting up short of the floor.
   *
  ...
  /**
   * Offers a Scanner if the pool is short of its floor when this process comes up.
   *
   * Without it the pool is only ever grown by a report, so a service that has just
   * started and has no traffic yet runs no Scanner at all — and the Scanner is the
   * only thing that force-advances the jobs of a runner that has gone silent (§8.3)
   * or retires the ones whose rounds are used up (§8.5). That is exactly the state
   * after a deploy, and exactly when a backlog left by the previous processes needs
   * someone to look at it. The spec leaves the floor to be reached by wake-ups
   * alone (§7); this is what actually gets there.
   *
   * Only below the floor, not on every boot: a rolling deploy would otherwise have
   * each replica offer one and fill the pool to its ceiling, which the ceiling
   * would cap but the idle-retire path would then have to unwind. Processes racing
   * on the same reading is harmless — `acquire` is one atomic script, so the
   * ceiling holds however many ask at once.
   *
   * A failure here is logged and swallowed. The pool is a hint about who is
   * scanning, not a thing this process needs in order to serve traffic, and an
   * unreachable Redis at boot must not stop the API from starting.
   */
  async onApplicationBootstrap(): Promise<void> {
    try {
      if (!(await this.pool.isBelowFloor())) {
        return
      }

      await this.scannerWakeup()
    } catch (error) {
      this.logger.warn(`Could not check the RatchetJQ Scanner pool at startup: ${error.message}`)
    }
  }
  ...
  private async runUntilRetired(signal: AbortSignal): Promise<void> {
    ...
    while (!this.isStopping && !signal.aborted) {
      ...
      // floor, which is what §7 asks for: the limit and the floor decide whether
      // a Scanner leaves, never whether it rests. Tying the rest to the counting
      // branch instead would leave a Scanner held at the floor rescanning with no
      // delay at all — a busy poll against Postgres in exactly the state where
      // one Scanner is all there is.
      if (advanced === 0) {
   ...
   * pool-size concurrency (`box-migration.manager.spec.ts`, the pool-exhaustion case).
   *
   ...
   * (`apps/runner/pkg/ratchetjq/poller.go:claimWithinBudget`). The difference matters here: the

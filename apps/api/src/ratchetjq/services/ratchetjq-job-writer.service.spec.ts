...
function makeWriter(
  options: {
    insertFails?: unknown
    rows?: Array<Partial<RatchetJQJob>>
    /** How many unfinished jobs the depth check finds for the executor instance. */
    unfinished?: number
  } = {},
) {
  // The depth check reads first and the insert writes second, so one stub answers
  // both: a `count(*)` shape for the former, rows for the latter.
  const query = jest.fn(async (sql: string) => {
    if (sql.includes('count(*)')) {
      return [{ unfinished: String(options.unfinished ?? 0) }]
    }
  ...
  const configService = { get: jest.fn(() => QUEUE_LIMIT) }

  const writer = new RatchetJQJobWriter({ query } as never, transfer as never, configService as never)

  const inserts = (): Array<[string, unknown[]]> =>
    query.mock.calls.filter(([sql]) => !String(sql).includes('count(*)')) as unknown as Array<[string, unknown[]]>
  const statement = (): [string, unknown[]] => inserts()[0]

  return { writer, query, transfer, configService, statement, inserts }
}
...
describe('RatchetJQJobWriter queue', () => {
  ...
  it('defaults the channel to NONE, the priority mid-range and the rollback to null', async () => {
    const { writer, statement } = makeWriter()
    ...
    expect(parameters[4]).toBe(RATCHETJQ_DEFAULT_PR)
    expect(parameters[9]).toBeNull()
  })

  // The point of not defaulting to 0: claims order by `pr ASC` and negatives are
  // refused, so 0 is the most urgent value there is. A default of 0 would leave a
  // caller no way to ask for anything ahead of ordinary work — this is the only
  // property of the number that the scheduler depends on.
  it('defaults above the most urgent priority, leaving room to outrank it', () => {
    expect(RATCHETJQ_DEFAULT_PR).toBeGreaterThan(0)
  })
...
describe('RatchetJQJobWriter submission checks', () => {
  ...
  // Zero is the most urgent priority, not a missing one — `?? RATCHETJQ_DEFAULT_PR`
  // and not `|| RATCHETJQ_DEFAULT_PR`, or the most urgent request would silently
  // become an ordinary one.
  it('accepts a priority of zero rather than reading it as absent', async () => {
    const { writer, statement } = makeWriter()
...
describe('RatchetJQJobWriter queue depth', () => {
  // The queue is otherwise unbounded: claims are capped per call and the runner
  // gates its own concurrency, but nothing stops `pending_run` growing faster than
  // either drains. Submission is the last moment the pressure can be handed back
  // to someone who can act on it.
  it('refuses a submission once the executor instance is at its limit', async () => {
    const { writer } = makeWriter({ unfinished: QUEUE_LIMIT })

    await expect(writer.queue(SUBMISSION)).rejects.toBeInstanceOf(RatchetJQQueueFullError)
  })

  it('writes no row for a submission it refused', async () => {
    const { writer, inserts } = makeWriter({ unfinished: QUEUE_LIMIT })

    await expect(writer.queue(SUBMISSION)).rejects.toThrow('at its limit')
    expect(inserts()).toHaveLength(0)
  })

  // The depth and the limit travel so a caller can tell how long to back off, and
  // so a limit being hit constantly reads as a tuning signal rather than an
  // incident.
  it('reports the depth it found and the limit it met', async () => {
    const { writer } = makeWriter({ unfinished: QUEUE_LIMIT + 5 })

    await expect(writer.queue(SUBMISSION)).rejects.toMatchObject({
      depth: { executor: 'runner', executorId: 'runner-1', unfinished: QUEUE_LIMIT + 5, limit: QUEUE_LIMIT },
    })
  })

  it('admits a submission while the queue is under the limit', async () => {
    const { writer, inserts } = makeWriter({ unfinished: QUEUE_LIMIT - 1 })

    await expect(writer.queue(SUBMISSION)).resolves.toMatchObject({ id: 'job-1' })
    expect(inserts()).toHaveLength(1)
  })

  // Counted per executor instance, not globally: what saturates is one runner's
  // queue, and a global ceiling would let a single wedged instance refuse
  // submissions aimed at every healthy one.
  it('counts only the unfinished rows of the instance being submitted to', async () => {
    const { writer, query } = makeWriter()
    ...
    expect(sql).toContain('"executor" = $1 AND "executorId" = $2 AND "period" <> $3')
    expect(parameters).toEqual(['runner', 'runner-1', RatchetJQJobPeriod.COMPLETED])
  })

  it('checks the pushed form too', async () => {
    const { writer } = makeWriter({ unfinished: QUEUE_LIMIT })

    await expect(writer.start(SUBMISSION)).rejects.toBeInstanceOf(RatchetJQQueueFullError)
  })

  // The one insert that must never be refused. A compensation exists because a
  // side effect already happened; turning it away over queue depth would leave
  // that side effect standing with nothing scheduled to undo it.
  it('never refuses a compensation, however deep the queue is', async () => {
    const { writer, inserts } = makeWriter({ unfinished: QUEUE_LIMIT * 10 })
    const rejected = { ...REJECTED_JOB, rollbackType: 'undo-demo' } as RatchetJQJob

    await expect(writer.queueRollbackFor(rejected, RatchetJQJobStatus.REJECTED)).resolves.toMatchObject({
      id: 'job-1',
    })
    expect(inserts()).toHaveLength(1)
  })
